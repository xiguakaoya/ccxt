...
module.exports = class okx extends Exchange {
  ...
  async fetchMarketsByType(type, params = {}) {
    ...
    if (type === "option") {
      const defaultUnderlying = this.safeValue(
        this.options,
        "defaultUnderlying",
        "BTC-USD"
      );
      ...
      if (currencyId === undefined) {
        throw new ArgumentsRequired(
          this.id +
            " fetchMarketsByType() requires an underlying uly or marketId parameter for options markets"
        );
      } else {
        request["uly"] = currencyId;
      }
    }
    const response = await this.publicGetPublicInstruments(
      ...
      for (let j = 0; j < chains.length; j++) {
        ...
        if (networkId !== undefined && networkId.indexOf("-") >= 0) {
          ...
          const precision = this.parsePrecision(
            this.safeString(chain, "wdTickSz")
          );
  ...
  async fetchOrderBook(symbol, limit = undefined, params = {}) {
    ...
    const response = await this.publicGetMarketBooks(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchTicker(symbol, params = {}) {
    ...
    const response = await this.publicGetMarketTicker(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchTickersByType(type, symbols = undefined, params = {}) {
    ...
    if (type === "option") {
      ...
      if (currencyId === undefined) {
        throw new ArgumentsRequired(
          this.id +
            " fetchTickersByType() requires an underlying uly or marketId parameter for options markets"
        );
      } else {
        request["uly"] = currencyId;
      }
  ...
  async fetchTrades(symbol, since = undefined, limit = undefined, params = {}) {
    ...
    const response = await this.publicGetMarketTrades(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchFundingRateHistory(
    ...
    if (symbol === undefined) {
      throw new ArgumentsRequired(
        this.id + " fetchFundingRateHistory() requires a symbol argument"
      );
    }
    await this.loadMarkets();
    const market = this.market(symbol);
    ...
    const response = await this.publicGetPublicFundingRateHistory(
      this.extend(request, params)
    );
    //
    //     {
  ...
  parseTradingFee(fee, market = undefined) {
    ...
    return {
      ...
      maker: this.parseNumber(
        Precise.stringNeg(this.safeString2(fee, "maker", "makerU"))
      ),
      taker: this.parseNumber(
        Precise.stringNeg(this.safeString2(fee, "taker", "takerU"))
      ),
    };
  }
  ...
  async fetchTradingFee(symbol, params = {}) {
    ...
    if (market["spot"]) {
      ...
      throw new NotSupported(
        this.id +
          " fetchTradingFee() supports spot, swap, future or option markets only"
      );
    }
    const response = await this.privateGetAccountTradeFee(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async createOrder(
    ...
    const triggerPrice = this.safeValueN(params, [
      "triggerPrice",
      "stopPrice",
      "triggerPx",
    ]);
    ...
    const stopLossPrice = this.safeValue2(
      params,
      "stopLossPrice",
      "slTriggerPx"
    );
    ...
    const defaultMarginMode = this.safeString2(
      this.options,
      "defaultMarginMode",
      "marginMode",
      "cross"
    ...
    if (margin === true && market["spot"] && !market["margin"]) {
      throw new NotSupported(
        this.id + " does not support margin trading for " + symbol + " market"
      );
    }
    if (spot) {
      if (margin) {
        const defaultCurrency =
          side === "buy" ? market["quote"] : market["base"];
    ...
    if (isMarketOrder || marketIOC) {
      ...
      if (spot && side === "buy") {
        ...
        if (tgtCcy === "quote_ccy") {
          ...
          const createMarketBuyOrderRequiresPrice = this.safeValue(
            this.options,
    ...
    if (postOnly) {
      ...
      if (takeProfitPrice !== undefined) {
        ...
        request["tpOrdPx"] =
          tpOrdPx === undefined ? "-1" : this.priceToPrecision(symbol, tpOrdPx);
        request["tpTriggerPxType"] = tpTriggerPxType;
      }
      if (stopLossPrice !== undefined) {
        request["slTriggerPx"] = this.priceToPrecision(symbol, stopLossPrice);
        request["slOrdPx"] =
          slOrdPx === undefined ? "-1" : this.priceToPrecision(symbol, slOrdPx);
    ...
    if (
      ...
      throw new ExchangeError(
        this.id +
          ' createOrder() this.options["createOrder"] must be either privatePostTradeBatchOrders or privatePostTradeOrder'
      );
    }
  ...
  async cancelOrder(id, symbol = undefined, params = {}) {
    ...
    const response = await this.privatePostTradeCancelOrder(
  ...
  async cancelOrders(ids, symbol = undefined, params = {}) {
    ...
    const clientOrderIds = this.parseIds(
      this.safeValue2(params, "clOrdId", "clientOrderId")
    );
  ...
  parseOrder(order, market = undefined) {
    ...
    const stopPrice = this.safeNumberN(order, [
      "triggerPx",
      "slTriggerPx",
      "tpTriggerPx",
    ]);
    ...
    if (stop || ordType in algoOrderTypes) {
      ...
      if (stop) {
        if (ordType === undefined) {
          throw new ArgumentsRequired(
            this.id +
              ' fetchOpenOrders() requires an "ordType" string parameter, "conditional", "oco", "trigger", "move_order_stop", "iceberg", or "twap"'
          );
        }
      }
    }
    ...
    if (stop || ordType in algoOrderTypes) {
      ...
      if (stop) {
        if (ordType === undefined) {
          throw new ArgumentsRequired(
            this.id +
              ' fetchCanceledOrders() requires an "ordType" string parameter, "conditional", "oco", "trigger", "move_order_stop", "iceberg", or "twap"'
          );
        }
    ...
    if (stop || ordType in algoOrderTypes) {
      ...
      if (stop) {
        if (ordType === undefined) {
          throw new ArgumentsRequired(
            this.id +
              ' fetchClosedOrders() requires an "ordType" string parameter, "conditional", "oco", "trigger", "move_order_stop", "iceberg", or "twap"'
          );
        }
      }
  ...
  async fetchMyTrades(
    ...
    const response = await this.privateGetTradeFillsHistory(
      this.extend(request, query)
    );
    //
    //     {
  ...
  async fetchDepositAddressesByNetwork(code, params = {}) {
    ...
    const response = await this.privateGetAssetDepositAddress(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchDepositAddress(code, params = {}) {
    ...
    if (network === undefined) {
      ...
      if (result === undefined) {
        ...
        if (result === undefined) {
          const defaultNetwork = this.safeString(
            this.options,
            "defaultNetwork",
            "ERC20"
          );
          ...
          if (result === undefined) {
            ...
            if (result === undefined) {
              throw new InvalidAddress(
                this.id +
                  " fetchDepositAddress() cannot find deposit address for " +
                  code
              );
            }
          }
  ...
  async withdraw(code, amount, address, tag = undefined, params = {}) {
    ...
    if (fee === undefined) {
      throw new ArgumentsRequired(
        this.id +
          " withdraw() requires a 'fee' string parameter, network transaction fee must be ≥ 0. Withdrawals to OKCoin or OKX are fee-free, please set '0'. Withdrawing to external digital asset address requires network transaction fee."
      );
    }
    const request = {
      ccy: currency["id"],
    ...
    if (!("pwd" in request)) {
      throw new ExchangeError(
        this.id +
          " withdraw() requires a password parameter or a pwd parameter, it must be the funding password, not the API passphrase"
      );
    }
    const response = await this.privatePostAssetWithdrawal(
      this.extend(request, query)
    );
    //
    //     {
  ...
  async fetchDeposits(
    ...
    const response = await this.privateGetAssetDepositHistory(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchWithdrawals(
    ...
    const response = await this.privateGetAssetWithdrawalHistory(
      this.extend(request, params)
    );
    //
    //     {
  ...
  parseTransaction(transaction, currency = undefined) {
    ...
    const status = this.parseTransactionStatus(
      this.safeString(transaction, "state")
    );
  ...
  async fetchLeverage(symbol, params = {}) {
    ...
    const response = await this.privateGetAccountLeverageInfo(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchPositions(symbols = undefined, params = {}) {
    ...
    const response = await this.privateGetAccountPositions(
      this.extend(request, query)
    );
    //
    //     {
  ...
  parsePosition(position, market = undefined) {
    ...
    if (market["inverse"]) {
      notionalString = Precise.stringDiv(
        Precise.stringMul(contractsAbs, contractSizeString),
        markPriceString
      );
    }
    ...
    let maintenanceMarginPercentage = Precise.stringDiv(
    ...
    if (initialMarginPercentage === undefined) {
      ...
      initialMarginString = Precise.stringMul(
    ...
    maintenanceMarginPercentage = this.parseNumber(
      Precise.stringDiv(
        Precise.stringAdd(maintenanceMarginPercentage, rounder),
        "1",
        4
      )
    );
    ...
    const percentage = this.parseNumber(
      Precise.stringMul(percentageString, "100")
    );
  ...
  async transfer(code, amount, fromAccount, toAccount, params = {}) {
    ...
    const response = await this.privatePostAssetTransfer(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchTransfer(id, code = undefined, params = {}) {
    ...
    const response = await this.privateGetAssetTransferState(
      this.extend(request, params)
    );
    //
    //     {
  ...
  sign(
    ...
    const request =
      "/api/" + this.version + "/" + this.implodeParams(path, params);
  ...
  parseFundingRate(contract, market = undefined) {
    ...
    //
    const nextFundingRateTimestamp = this.safeInteger(
      contract,
      "nextFundingTime"
    );
  ...
  async fetchFundingRate(symbol, params = {}) {
    ...
    if (!market["swap"]) {
      throw new ExchangeError(
        this.id + " fetchFundingRate() is only valid for swap markets"
      );
    }
    const request = {
      instId: market["id"],
    };
    const response = await this.publicGetPublicFundingRate(
      this.extend(request, params)
    );
    //
    //    {
  ...
  async fetchFundingHistory(
    ...
    const response = await this.privateGetAccountBillsArchive(
      this.extend(request, query)
    );
    //
    //    {
  ...
  async setLeverage(leverage, symbol = undefined, params = {}) {
    ...
    if (symbol === undefined) {
      throw new ArgumentsRequired(
        this.id + " setLeverage() requires a symbol argument"
      );
    }
    ...
    if (leverage < 1 || leverage > 125) {
      throw new BadRequest(
        this.id + " setLeverage() leverage should be between 1 and 125"
      );
    }
    await this.loadMarkets();
    const market = this.market(symbol);
    ...
    if (marginMode === "isolated") {
      if (posSide === undefined) {
        throw new ArgumentsRequired(
          this.id +
            " setLeverage() requires a posSide argument for isolated margin"
        );
      }
      if (posSide !== "long" && posSide !== "short") {
        throw new BadRequest(
          this.id +
            ' setLeverage() requires the posSide argument to be either "long" or "short"'
        );
      }
    }
    const response = await this.privatePostAccountSetLeverage(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async setPositionMode(hedged, symbol = undefined, params = {}) {
    ...
    const response = await this.privatePostAccountSetPositionMode(
      this.extend(request, params)
    );
    //
    //    {
  ...
  async setMarginMode(marginMode, symbol = undefined, params = {}) {
    ...
    if (symbol === undefined) {
      throw new ArgumentsRequired(
        this.id + " setMarginMode() requires a symbol argument"
      );
    }
    // WARNING: THIS WILL INCREASE LIQUIDATION PRICE FOR OPEN ISOLATED LONG POSITIONS
    // AND DECREASE LIQUIDATION PRICE FOR OPEN ISOLATED SHORT POSITIONS
    ...
    if (marginMode !== "cross" && marginMode !== "isolated") {
      throw new BadRequest(
        this.id + " setMarginMode() marginMode must be either cross or isolated"
      );
    }
    await this.loadMarkets();
    const market = this.market(symbol);
    ...
    if (lever === undefined || lever < 1 || lever > 125) {
      throw new BadRequest(
        this.id + ' setMarginMode() params["lever"] should be between 1 and 125'
      );
    }
  ...
  async fetchBorrowRate(code, params = {}) {
    ...
    const response = await this.privateGetAccountInterestRate(
      this.extend(request, params)
    );
    //
    //    {
  ...
  async fetchBorrowRateHistories(
    ...
    const response = await this.publicGetAssetLendingRateHistory(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async modifyMarginHelper(symbol, amount, type, params = {}) {
    ...
    const response = await this.privatePostAccountPositionMarginBalance(
      this.extend(request, params)
    );
    //
    //     {
  ...
  parseMarginModification(data, market = undefined) {
    ...
    const code = responseMarket["inverse"]
      ? responseMarket["base"]
      : responseMarket["quote"];
    return {
      info: data,
      type: type,
      amount: amountRaw,
  ...
  async fetchMarketLeverageTiers(symbol, params = {}) {
    ...
    await this.loadMarkets();
    const market = this.market(symbol);
    const type = market["spot"]
      ? "MARGIN"
      : this.convertToInstrumentType(market["type"]);
    const uly = this.safeString(market["info"], "uly");
    if (!uly) {
      if (type !== "MARGIN") {
        throw new BadRequest(
          this.id +
            " fetchMarketLeverageTiers() cannot fetch leverage tiers for " +
            symbol
        );
      }
    }
    ...
    const response = await this.publicGetPublicPositionTiers(
      this.extend(request, params)
    );
    //
    //    {
  ...
  async fetchBorrowInterest(
    ...
    const response = await this.privateGetAccountInterestAccrued(
      this.extend(request, params)
    );
    //
    //    {
  ...
  async borrowMargin(code, amount, symbol = undefined, params = {}) {
    ...
    const response = await this.privatePostAccountBorrowRepay(
      this.extend(request, params)
    );
    //
    //     {
  ...
  async fetchOpenInterestHistory(
    ...
    if (timeframe !== "5m" && timeframe !== "1H" && timeframe !== "1D") {
      throw new BadRequest(
        this.id +
          " fetchOpenInterestHistory cannot only use the 5m, 1h, and 1d timeframe"
      );
    }
    await this.loadMarkets();
    ...
    const response = await this.publicGetRubikStatContractsOpenInterestVolume(
      this.extend(request, params)
    );
    //
    //    {
