...
const { TRUNCATE, ROUND, DECIMAL_PLACES, NO_PADDING, TICK_SIZE } =
...
module.exports = class Exchange {
  ...
  constructor(userConfig = {}) {
    ...
    const configEntries = Object.entries(this.describe()).concat(
        ...
        .catch((e) => {
          if (isNode) {
            throw new ExchangeNotAvailable(
              [this.id, method, url, e.type, e.message].join(" ")
            );
          }
          throw e; // rethrow all unknown errors
        })
        .then((response) =>
          this.handleRestResponse(response, url, method, headers, body)
        );
  ...
  defineRestApiEndpoint(
    ...
    const underscorePrefix = [paths[0]]
      ...
      .join("_");
    const camelcase =
      camelcasePrefix + camelcaseMethod + this.capitalize(camelcaseSuffix);
    const underscore =
      underscorePrefix + "_" + lowercaseMethod + "_" + underscoreSuffix;
  ...
  onJsonResponse(responseBody) {
    return this.quoteJsonNumbers
      ? responseBody.replace(/":([+.0-9eE-]+)([,}])/g, '":"$1"$2')
      : responseBody;
  }
  ...
  hashMessage(message) {
    ...
    return (
      "0x" +
      this.hash(this.binaryConcat(prefix, binaryMessage), "keccak", "hex")
    );
  }
  ...
  checkOrderArguments(market, type, side, amount, price, params) {
    ...
    if (amount <= 0) {
      throw new ArgumentsRequired(
        this.id + " createOrder() amount should be above 0"
      );
    }
  }
    ...
    if (Array.isArray(orders)) {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        const order = this.extend(
          this.parseOrder(this.extend({ id: id }, orders[id]), market),
          params
        );
        results.push(order);
      }
    ...
    for (let i = 0; i < fees.length; i++) {
      ...
      if (feeCurrencyCode !== undefined) {
        ...
        if (rateKey in reduced[feeCurrencyCode]) {
          reduced[feeCurrencyCode][rateKey]["cost"] = Precise.stringAdd(
  ...
  getNetwork(network, code) {
    ...
    if (network === code) {
      ...
      throw new NotSupported(
        this.id + " network " + network + " is not yet supported"
      );
    }
  }
  ...
  parseLeverageTiers(response, symbols = undefined, marketIdKey = undefined) {
    ...
    for (let i = 0; i < response.length; i++) {
      ...
      if (
        contract &&
        (symbols === undefined || this.inArray(symbol, symbols))
      ) {
  ...
  parseTransactions(
    ...
    for (let i = 0; i < transactions.length; i++) {
      const transaction = this.extend(
        this.parseTransaction(transactions[i], currency),
        params
      );
      ...
      if (this.requiredCredentials[key] && !this[key]) {
        if (error) {
          throw new AuthenticationError(
            this.id + ' requires "' + key + '" credential'
          );
        } else {
          return error;
        }
  ...
  async fetchTransactionFee(code, params = {}) {
    if (!this.has["fetchTransactionFees"]) {
      throw new NotSupported(
        this.id + " fetchTransactionFee() is not supported yet"
      );
    }
  ...
  async fetchTransactionFees(codes = undefined, params = {}) {
    throw new NotSupported(
      this.id + " fetchTransactionFees() is not supported yet"
    );
  }
  ...
  getSupportedMapping(key, mapping = {}) {
    if (key in mapping) {
      ...
      throw new NotSupported(
        this.id + " " + key + " does not have a value in mapping"
      );
    }
  }
  ...
  async fetchBorrowRate(code, params = {}) {
    ...
    if (!this.has["fetchBorrowRates"]) {
      throw new NotSupported(
        this.id + " fetchBorrowRate() is not supported yet"
      );
    }
  ...
  handleSubTypeAndParams(methodName, market = undefined, params = {}) {
    ...
    if (subTypeInParams !== undefined) {
      ...
      if (subType === undefined) {
        const exchangeWideValue = this.safeString2(
          this.options,
          "defaultSubType",
          "subType",
          "linear"
    ...
    if (this.has["fetchTickers"]) {
      ...
      if (ticker === undefined) {
        throw new NullResponse(
          this.id + " fetchTickers() could not find a ticker for " + symbol
        );
      } else {
        return ticker;
      }
  ...
  async fetchClosedOrders(
    ...
    throw new NotSupported(
      this.id + " fetchClosedOrders() is not supported yet"
    );
  }
  ...
  async fetchTransactions(
    ...
    throw new NotSupported(
      this.id + " fetchTransactions() is not supported yet"
    );
  }
  ...
  async fetchDepositAddress(code, params = {}) {
    if (this.has["fetchDepositAddresses"]) {
      ...
      throw new NotSupported(
        this.id + " fetchDepositAddress() is not supported yet"
      );
    }
  }
  ...
  async fetchMarketLeverageTiers(symbol, params = {}) {
    if (this.has["fetchLeverageTiers"]) {
      ...
      if (!market["contract"]) {
        throw new BadSymbol(
          this.id + " fetchMarketLeverageTiers() supports contract markets only"
        );
      }
      ...
      throw new NotSupported(
        this.id + " fetchMarketLeverageTiers() is not supported yet"
      );
    }
  }
  ...
  async createPostOnlyOrder(symbol, type, side, amount, price, params = {}) {
    if (!this.has["createPostOnlyOrder"]) {
      throw new NotSupported(
        this.id + "createPostOnlyOrder() is not supported yet"
      );
    }
  ...
  async createReduceOnlyOrder(symbol, type, side, amount, price, params = {}) {
    if (!this.has["createReduceOnlyOrder"]) {
      throw new NotSupported(
        this.id + "createReduceOnlyOrder() is not supported yet"
      );
    }
  ...
  async createStopOrder(
    ...
    if (!this.has["createStopOrder"]) {
      throw new NotSupported(
        this.id + " createStopOrder() is not supported yet"
      );
    }
    if (stopPrice === undefined) {
      throw new ArgumentsRequired(
        this.id + " create_stop_order() requires a stopPrice argument"
      );
    }
  ...
  async createStopLimitOrder(
    ...
    if (!this.has["createStopLimitOrder"]) {
      throw new NotSupported(
        this.id + " createStopLimitOrder() is not supported yet"
      );
    }
    const query = this.extend(params, { stopPrice: stopPrice });
  ...
  async createStopMarketOrder(symbol, side, amount, stopPrice, params = {}) {
    if (!this.has["createStopMarketOrder"]) {
      throw new NotSupported(
        this.id + " createStopMarketOrder() is not supported yet"
      );
    }
    const query = this.extend(params, { stopPrice: stopPrice });
    return await this.createOrder(
  ...
  async fetchTradingFee(symbol, params = {}) {
    if (!this.has["fetchTradingFees"]) {
      throw new NotSupported(
        this.id + " fetchTradingFee() is not supported yet"
      );
    }
  ...
  parseOpenInterest(interest, market = undefined) {
    throw new NotSupported(
      this.id + " parseOpenInterest () is not supported yet"
    );
  }
  ...
  async fetchFundingRate(symbol, params = {}) {
    if (this.has["fetchFundingRates"]) {
      ...
      if (!market["contract"]) {
        throw new BadSymbol(
          this.id + " fetchFundingRate() supports contract markets only"
        );
      }
      ...
      if (rate === undefined) {
        throw new NullResponse(
          this.id + " fetchFundingRate () returned no data for " + symbol
        );
      } else {
        return rate;
      }
      ...
      throw new NotSupported(
        this.id + " fetchFundingRate () is not supported yet"
      );
    }
  }
  ...
  async fetchMarkOHLCV(
    ...
    if (this.has["fetchMarkOHLCV"]) {
      ...
      throw new NotSupported(
        this.id + " fetchMarkOHLCV () is not supported yet"
      );
    }
  }
  ...
  async fetchIndexOHLCV(
    ...
    if (this.has["fetchIndexOHLCV"]) {
      ...
      throw new NotSupported(
        this.id + " fetchIndexOHLCV () is not supported yet"
      );
    }
  }
  ...
  async fetchPremiumIndexOHLCV(
    ...
    if (this.has["fetchPremiumIndexOHLCV"]) {
      ...
      throw new NotSupported(
        this.id + " fetchPremiumIndexOHLCV () is not supported yet"
      );
    }
  }
  ...
  handleTimeInForce(params = {}) {
    ...
    if (timeInForce !== undefined) {
      ...
      if (exchangeValue === undefined) {
        throw new ExchangeError(
          this.id + ' does not support timeInForce "' + timeInForce + '"'
        );
      }
  ...
  handleMarginModeAndParams(methodName, params = {}) {
    ...
    const methodOptions = this.safeValue(this.options, methodName, {});
    const methodMarginMode = this.safeString2(
      methodOptions,
      "marginMode",
      "defaultMarginMode",
      defaultMarginMode
    );
    const marginMode = this.safeStringLower2(
      params,
      "marginMode",
      "defaultMarginMode",
      methodMarginMode
